...
function TechnicalsReport() {
  ...
  const { payments, loading, error, fetchPayments, addPayment } = useLaboratoryPaymentStore();

  const [searchQuery, setSearchQuery] = useState('');
  const [showAddPaymentModal, setShowAddPaymentModal] = useState(false);
  const [newPaymentData, setNewPaymentData] = useState({ technicianId: '', amount: '' });
  ...
  const handleAddPaymentSubmit = async (e) => {
    ...
    try {
      ...
      setAddPaymentError("Ödəniş əlavə edilərkən xəta baş verdi: " + (err.message || 'Bilinməyən xəta'));
    }
  };
  ...
  return (
    <div className="sentOrdersContainer">
      ...
      {loading && <p style={{ textAlign: 'center', margin: '20px 0', color: '#007bff' }}>Məlumatlar yüklənir...</p>}
      {error && <p style={{ textAlign: 'center', margin: '20px 0', color: 'red' }}>Xəta: {error}</p>}
        ...
        <table className="labTable">
          ...
          <tbody>
            ...
            {filteredData.length === 0 && !loading && !error ? (
              <tr>
                <td colSpan={tableHead.length + (icons.length > 0 ? 1 : 0) + 1} style={{ textAlign: "center" }}>Heç bir istifadəçi tapılmadı.</td>
              </tr>
            ) : (
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <form onSubmit={handleAddPaymentSubmit}>
              ...
              {addPaymentError && <p style={{ color: 'red', marginBottom: '10px' }}>{addPaymentError}</p>}
              {addPaymentSuccess && <p style={{ color: 'green', marginBottom: '10px' }}>Ödəniş uğurla əlavə edildi!</p>}
              <div className="modal-actions">
                <button type="submit" className="submit-btn">Əlavə Et</button>
