...
const PatientInsuranceBalanceAdd = () => {
    ...
    const [formData, setFormData] = useState({
        date: '',
        amount: '',
        documents: [], // Changed to an array to store multiple document URLs
    });
    ...
    const handleFileSelect = (event) => {
        ...
        setFormData((prev) => ({
            ...prev,
            documents: [...prev.documents, ...newImageUrls], // Add new URLs to existing ones
        }));
    ...
    const handleImageClick = (fileUrl) => {
        setSelectedImage(fileUrl); // Set the clicked image for modal display
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (validateForm()) {
            ...
            // For file uploads, you'd typically use FormData for the API request:
            /*
            ...
            formData.documents.forEach((docUrl, index) => {
                ...
                // This part requires more complex handling or changing how files are stored initially.
                // For a simple demo, we just acknowledge the URLs.
                console.log(`Document URL for backend: ${docUrl}`); 
            });
            // axios.post('/api/your-endpoint', dataToSend, { headers: { 'Content-Type': 'multipart/form-data' } })
            */

            alert('Məlumatlar uğurla yadda saxlandı!');
    ...
    const handleCancel = () => {
        // Revoke all object URLs when cancelling to prevent memory leaks
                    ...
                    <div className="flex items-start gap-6">
                        ...
                        <div className="flex-grow flex flex-col gap-3">
                         ...
                         <button
                                ...
                                className="inline-flex items-center justify-center w-[200px] px-4 py-2 text-[#697586] text-[#fff] bg-[#155EEF] border border-gray-300 rounded-[8px] shadow-sm text-sm font-medium cursor-pointer hover:bg-[#153EEF] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#697586]"
                            >
