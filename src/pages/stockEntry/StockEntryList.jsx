...
const fetchWorkerDetails = async (workerId) => {
 ...
 if (!response.ok) {
  console.error(`API Error: Failed to fetch worker details for ID ${workerId}. Status: ${response.status}`);
...
useEffect(() => {
 ...
 const fetchWorkerData = async () => {
  ...
  console.log("Orders massivi yeniləndi. Yüklənməli unikal ID-lər:", uniqueWorkerIds);
...
const formattedData = useMemo(() => {
 ...
 const workerName = workerDetails[order.personWhoPlacedOrder] || "Yüklənir...";
 return {
  id: order.id,
  rowNumber: index + 1,
  date: order.date || "-",
