    ...
    <div className="operationPictures-container">
      ...
      <div className="operationPictures-table-section">
        <table className="operationPictures-table">
            ...
            <tr>
              <th>
                <span className="operationPictures-firstElementOfTHS">
                  <HiArrowsUpDown style={{marginRight: 4}} /> {operationPicturesData.length===0?'0':`1-${operationPicturesData.length}`}
                </span>
              </th>
              <th>
                <span>
          ...
          <tbody>
            {operationPicturesData.map((row, idx) => (
              <tr key={row.id}>
                <td>{idx + 1}</td>
                <td>{row.name}</td>
                <td>
            ...
            {/* Boş sətirlər */}
            {Array.from({ length: 8 - operationPicturesData.length }).map((_, i) => (
              <tr key={"empty-" + i}>
                <td>&nbsp;</td>
                <td></td>
                <td></td>
