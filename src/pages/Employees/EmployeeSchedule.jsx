...
function EmployeeSchedule() {
  ...
  const fetchDoctors = useCallback(async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/general-calendar/read-doctors`);
  ...
  const fetchSchedules = useCallback(async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE_URL}/workers-work-schedule/read`);
  ...
  const searchSchedules = async (weekDay) => {
    try {
      setLoading(true);
      const response = await axios.post(`${API_BASE_URL}/workers-work-schedule/search`, {
        weekDay: weekDay
      });
      setSchedules(response.data);
      setError(null);
                  ...
                  {employeeBlocks
                    ...
                    .map((block) => {
                      ...
                      return (
                        <div
                          key={block.id}
                          className={`employee-full-block employee-color-${block.employeeId % 5} ${isSelected ? 'selected' : ''}`}
                          style={{
                            top: `${topPosition}px`,
                            height: `${height}px`,
                            width: `${block.width}%`,
                        ...
                        >
                          ...
                          <div className="employee-name">{block.employee.name} {block.employee.surname}</div>
